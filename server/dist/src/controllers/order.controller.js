import { z } from "zod";
import { orderingFood, getOrdersByID, getStatus, updateOrderStatus, } from "../services/order.services";
...
async function gettingStatusOfOrder(req, res) {
    try {
        const id = Number(req.params.id);
...
async function updatingStatusOfOrder(req, res) {
    try {
        const id = Number(req.params.id);
